...
Заметки и настройки лежат в `~/Library/Application Support/com.dmytrofiliurskyi.stickies` (на Linux `~/.local/share/stickies`), папку заметок можно перенести куда угодно в настройках
...
- [x] **Десктопные окна** - безрамочные always-on-bottom стикеры с drag и resize-уголком
- [x] **Редактура в vim** - `$EDITOR` в собственном терминале, каждый save синхронизируется
- [x] **Выбор терминала** - автоопределенный список плюс своя команда
- [x] **Markdown-превью** - офлайн-рендеринг с зачеркиванием, таблицами и кодом
- [x] **Встроенный редактор** - inline-правки markdown-исходника через Ctrl+Enter
- [x] **Приложение меню-бара** - tray-меню, без Dock-иконки, красный крестик прячет вместо quit
- [x] **Прикрепленный вид** - кнопка вкл/выкл, цвет кнопки, цвета заметок
- [x] **Своя папка** - папка заметок куда угодно с миграцией
- [x] **Show Desktop safe** - Stationary-окна переживают macOS горячий угол
- [x] **Release CI** - draft-релизы для macOS и Linux по version-тегам

### Идеи

- [ ] **Windows-поддержка** - vim-флоу и запуск терминала на Windows
- [ ] **Подписанные macOS-сборки** - Developer ID и нотаризация, никакого `xattr`
- [ ] **Глобальный хоткей** - новая заметка откуда угодно без менеджера
- [ ] **Автозапуск** - старт при входе с восстановленными заметками
- [ ] **Поиск** - полнотекстовый поиск по заметкам из менеджера
